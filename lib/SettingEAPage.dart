...
class SettingEAPage extends StatefulWidget {
  const SettingEAPage({Key? key}) : super(key: key);

  @override
  _SettingEAPageState createState() => _SettingEAPageState();
}

class _SettingEAPageState extends State<SettingEAPage> {
  int _selectedEAIndex = -1;
  static const String selectedButtonIndexKey = 'selectedEAIndex';
  ...
  Future<void> _loadSelectedIndex() async {
    ...
    setState(() {
      _selectedEAIndex = prefs.getInt(selectedButtonIndexKey) ?? 0;
    });
  }
            ...
            children: [
              Center(
                child: SvgPicture.asset(
                  'assets/icons/settingExpression.svg', // 使用适用于暗黑模式设置的图标
  ...
  Widget _buildButton(int index, String text) {
    return Padding(
      ...
      child: CustomButton(
        text: text,
        isSelected: _selectedEAIndex == index,
        onPressed: () {
          setState(() {
            _selectedEAIndex = index;
          });
